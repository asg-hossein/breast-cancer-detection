...
Replaces all previous pipelines
"""
import sys
import os
import argparse
    ...
    while True:
        try:
            ...
            if choice in mode_mapping:
                ...
                return selected_mode, use_pca, remove_outliers
            else:
...
def parse_arguments():
    if len(sys.argv) == 1:
        mode, use_pca, remove_outliers = display_interactive_menu()
...
def run_preprocessed_pipeline(use_pca=False, remove_outliers=True, output_prefix=''):
    ...
    prefix = f"preprocessed_{pca_str}_{timestamp}"
...
def run_fuzzy_pipeline(use_pca=False, output_prefix=''):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    pca_str = "with_pca" if use_pca else "no_pca"
    prefix = f"fuzzy_{pca_str}_{timestamp}"
...
def run_full_analysis_pipeline(output_prefix=''):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    prefix = f"full_analysis_{timestamp}"
...
def main():
    ...
    if args.mode == 'basic' or args.mode == 'all':
        all_results['basic'] = run_basic_pipeline()
    ...
    if args.mode == 'full' or args.mode == 'all':
        all_results['full'] = run_full_analysis_pipeline()
